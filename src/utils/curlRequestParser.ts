...
export class CurlRequestParser implements RequestParser {
    ...
    public constructor(private readonly requestRawText: string, private readonly document: TextDocument, private readonly settings: IRestClientSettings) {
    }
    ...
    public async parseHttpRequest(name?: string): Promise<HttpRequest> {
        ...
        if (typeof body === 'string' && body[0] === '@') {
            const fileAbsolutePath = await resolveRequestBodyPath(body.substring(1), this.document);
            if (fileAbsolutePath) {
