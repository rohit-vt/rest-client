...
export class Selector {
    ...
    public static async getRequest(document: TextDocument, selection?: Selection, range: Range | null = null): Promise<SelectedRequest | null> {
        let selectedText: string | null;
        if (!selection || selection.isEmpty || range) {
            const activeLine = range?.start.line ?? selection?.active.line ?? 0;
