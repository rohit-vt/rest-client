import { ExtensionContext, Range, TextDocument, window } from 'vscode';
...
export class RequestController {
    ...
    public async run(range?: Range, document: TextDocument | undefined = getCurrentTextDocument()) {
        ...
        const selection = editor?.document === document ? editor.selection : undefined;

        const selectedRequest = await Selector.getRequest(document, selection, range);
        if (!selectedRequest) {
            return;
        }
        ...
        try {
            ...
            try {
                if (settings.previewResponseInUntitledDocument) {
                    await this._textDocumentView.render(response, settings.previewColumn);
                } else {
                    await this._webview.render(response, settings.previewColumn);
                }
            } catch (reason) {
