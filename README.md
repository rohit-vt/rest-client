...
* Requests, variable resolution and document symbols operate on the document the request came from, instead of
  reading `window.activeTextEditor`. That coupling made "Send Request" silently do nothing, and threw
  `TypeError: Cannot read properties of undefined (reading 'uri')` from the document symbol provider whenever
  no editor was active.
* `document` is required rather than optional in the `HttpVariableProvider` interface, which is what let
  `undefined` reach that call in the first place.
* Response preview honours the `rest-client.previewColumn` setting directly instead of computing
  `activeColumn + 1`, so `beside` opens a real split.
* Preview rendering is awaited, so a failure inside it is reported instead of becoming an unhandled rejection.

### Known issue

The webview response preview does not survive in Cursor — the panel is created, revealed, then immediately
disposed by the host. Set `"rest-client.previewResponseInUntitledDocument": true` to render responses as a
text document instead. Unresolved.
