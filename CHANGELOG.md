...
* __Bug Fix__: Resolve requests against their own document instead of `window.activeTextEditor`, fixing silent
  ...
  symbol provider
* __Bug Fix__: Honour `previewColumn` directly so `beside` opens a split view
* __Bug Fix__: Await response preview rendering so failures surface instead of becoming unhandled rejections
